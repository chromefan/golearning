...
func main() {
	url := "bdnetdisk://n/action.OPEN_FILE?m_n_v=10.1.40&uk=2371440020&fsid=746384669587786&video_progress=234234&username=xxx"
	str := encodeUrl(url)
	/*noteId := int64(15688084068531868)
	shortUrl := createShortUrl(noteId)
	shortUrl = "5MTU2NzU4NzYyMjA3NDg-MTU"
	shortUrl2 := "MTU2NzU4NzYyMjA3NDg-MTU+"
	noteIdFrom := fromShortUrl(shortUrl)
	noteIdFrom = ShortUrlDecode(shortUrl2)*/
	fmt.Println(str)
}

func encodeUrl(str string) string  {
	b := []byte(str)
	encodeString := StdEncoding.EncodeToString(b)
	shortUrl := url.QueryEscape(encodeString)
	fmt.Println(shortUrl)
	return shortUrl
}
func createShortUrl(noteId int64) string  {
	str := fmt.Sprintf("%d",noteId)
	b := []byte(str)
	encodeString := StdEncoding.EncodeToString(b)
	shortUrl := encodeString
	fmt.Println(shortUrl)
	return shortUrl
}
func fromShortUrl(url string)  int64{
	decodeByte ,_ := StdEncoding.DecodeString(url)
	decodeString := string(decodeByte)
	NoteId, _ := strconv.ParseInt(decodeString, 10, 64)
	return NoteId
}
//根据短链解析notID
func ShortUrlDecode(surl string) int64 {
	surl = html.EscapeString(surl)
	//fmt.Println(surl)
	decodeByte ,_ := StdEncoding.DecodeString(surl)
	shortUrl := string(decodeByte)
	NoteId, _ := strconv.ParseInt(shortUrl, 10, 64)
	return NoteId
}